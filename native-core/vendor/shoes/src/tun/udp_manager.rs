...
/// DNS queries over UDP cannot be carried by TCP-only proxy chains such as
            ...
            packet = rx.recv() => {
                ...
                if dest_addr.port() == 53 {
                    ...
                    tokio::spawn(async move {
                        match dns_query_over_tcp(dest_addr, payload, proxy_selector, resolver).await {
                            Ok(response) => {
                                if response_tx.try_send((response, dest_addr, peer_addr)).is_err() {
                                    debug!(
                                        "[TunUdpSession {}] DNS response channel full, dropping response from {}",
                                        peer_addr, dest_addr
                                    );
                                }
                            }
                            Err(e) => {
                                debug!(
                                    "[TunUdpSession {}] DNS-over-TCP failed for {}: {}",
...
async fn dns_query_over_tcp(
    ...
    if payload.len() > u16::MAX as usize {
        return Err(io::Error::new(
            ...
            "DNS payload too large",
        ));
    }
    let target = socket_addr_to_net_location(dns_addr);
    ...
    let (chain_group, remote_location) = match decision {
        ...
        ConnectDecision::Block => {
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                "DNS destination blocked",
            ));
        }
    };

    let fut = async move {
        let mut setup = chain_group.connect_tcp(remote_location, &resolver).await?;
        let len = payload.len() as u16;
        setup.client_stream.write_all(&len.to_be_bytes()).await?;
        setup.client_stream.write_all(&payload).await?;
        setup.client_stream.flush().await?;

        let mut len_buf = [0u8; 2];
        setup.client_stream.read_exact(&mut len_buf).await?;
        let response_len = u16::from_be_bytes(len_buf) as usize;
        let mut response = vec![0u8; response_len];
        setup.client_stream.read_exact(&mut response).await?;
        Ok(response)
    };

    tokio::time::timeout(DNS_TCP_TIMEOUT, fut)
        .await
        .map_err(|_| io::Error::new(io::ErrorKind::TimedOut, "DNS-over-TCP timed out"))?
}
