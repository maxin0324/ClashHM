...
- Parses `proxies`, `proxy-groups`, `rules`, and provider-expanded local `proxy-providers[].proxies` when present.
- Parses common Clash `rules` and maps `MATCH`, `DOMAIN`, `DOMAIN-SUFFIX`, `IP-CIDR`, `IP-CIDR6`, and `DST-PORT` to shoes TUN rules.
- Skips currently unsupported routing rules such as `DOMAIN-KEYWORD`, `GEOIP`, `GEOSITE`, and `RULE-SET`, then relies on the Clash `MATCH` rule or generated default rule for fallback routing.
