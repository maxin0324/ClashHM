...
- Fails explicitly for unsupported outbound protocols or transports such as Hysteria2, TUIC, `network: grpc`, and `network: h2`. The vendored shoes backend currently has Hysteria2/TUIC server-side code but no client outbound config/handler, and Clash/Xray `network: h2` is not the same protocol as shoes/sing-box h2mux, so these must not be silently mapped to TCP or h2mux.
