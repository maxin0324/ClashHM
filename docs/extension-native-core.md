...
- Unsupported routing rules that the embedded shoes rule mask cannot model, including `DOMAIN-KEYWORD`, `GEOIP`, `GEOSITE`, and unexpanded/advanced `RULE-SET`, are not represented as fake rules. The adapter currently skips them and relies on the Clash `MATCH` rule or generated default rule; full parity requires backend rule-matcher work.
