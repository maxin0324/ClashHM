...
- The adapter supports `direct`, Shadowsocks, Snell, AnyTLS, NaiveProxy, SOCKS5, HTTP, VMess, VLESS, Trojan, Hysteria2, TUIC, TLS/WS/HTTP2/gRPC/Reality/ShadowTLS/v2ray-plugin WebSocket wrapping, and `mux`/`h2mux` options for VMess/VLESS/Trojan. It handles common Clash block options such as `ws-opts.path`, `ws-opts.headers.Host`, `h2-opts.path`, `h2-opts.host`, `grpc-opts.serviceName`, `reality-opts.public-key`, `reality-opts.short-id`, `reality-opts.server-name`, ShadowTLS `plugin-opts`, v2ray-plugin `plugin-opts`, and `udp: false`.
- Flow-style proxy groups such as `proxies: [A, B, DIRECT]` are parsed without truncating at list commas. The ArkTS config parser uses the same nested-aware flow parsing for proxy display and provider expansion. The native parser also expands YAML-native inline `proxy-providers[].proxies` through group `use:` entries and local/inline `rule-providers[].payload` or `rule-providers[].rules` through common `RULE-SET` entries.
- Unsupported protocols return explicit adapter errors. Hysteria2 and TUIC now have initial TCP/UDP client paths and are not silently treated as plain TCP. Clash/Xray `network: h2` and `network: grpc` now use real HTTP/2-based transport wrappers and are not shoes/sing-box h2mux.
...
- Subscription update materializes remote `proxy-providers` and `rule-providers` to local files when the provider has a `url`. `ConfigManager.generateMergedConfig` expands local provider nodes and expands common `RULE-SET` entries from `domain`, `ipcidr`, and `classical` rule providers before sending config to the Extension. Provider definitions support block-style and common flow-style entries such as `provider: { path: ./provider.yaml, behavior: domain }`; provider files can use wrapped `proxies:` / `payload:` / `rules:` sections, inline flow arrays, or direct top-level lists. Expanded rule-provider entries include `DOMAIN`, `DOMAIN-SUFFIX`, `DOMAIN-KEYWORD`, `IP-CIDR`, `IP-CIDR6`, `DST-PORT`, `GEOIP`, and `GEOSITE`; domain providers also accept `domain:`, `full:`, and `keyword:` prefixes.
- The adapter maps common Clash rules to shoes TUN rules: `MATCH`, `DOMAIN`, `DOMAIN-SUFFIX`, `DOMAIN-KEYWORD`, `IP-CIDR`, `IP-CIDR6`, `DST-PORT`, basic `GEOIP,PRIVATE/LAN`, MMDB-backed `GEOIP,<country-code>`, built-in private `GEOSITE,cn/private/local/lan`, and dat-backed `GEOSITE,<category>` entries.
- Unsupported routing rules that the embedded shoes rule matcher still cannot model, including unresolved dynamic `RULE-SET` forms and GEOSITE regex entries, are not represented as fake rules. The adapter currently skips them and relies on the Clash `MATCH` rule or generated default rule; status JSON reports `skippedRuleCount` and `skippedRuleTypes` so the UI/logs can explain partial rule coverage. Full parity requires backend rule-matcher work.
- Hysteria2 and TUIC support is implemented in the vendored backend client path, not as adapter-only YAML aliases. Remaining work is real-device validation and broader option compatibility.
...
- `url-test` and `fallback` groups update their selected node from native-core latency results. The probe builds a single-node shoes client chain, connects through that proxy path to the configured URL, performs an HTTP/HTTPS GET, and treats 2xx/3xx response status as success. This covers proxy protocol handshake, TLS for HTTPS test URLs, and HTTP response validation.
...
- `testDelay` performs a native proxy-chain HTTP URL-test against the configured test URL, caches the result for `getProxies`, and feeds cached results into `url-test` / `fallback` group selection. The same probe can run locally while disconnected after loading config into native-core.
...
- `rule-providers` when `RULE-SET` entries can be expanded from local `domain`, `ipcidr`, or `classical` provider payloads, including common inline and flow-list payloads
- `proxy-groups` with `select`, `url-test`, `fallback`
- `rules` with `MATCH`, `DOMAIN`, `DOMAIN-SUFFIX`, `DOMAIN-KEYWORD`, `IP-CIDR`, basic `GEOIP,PRIVATE/LAN`, MMDB-backed `GEOIP,<country-code>`, built-in private `GEOSITE,cn/private/local/lan`, and dat-backed `GEOSITE,<category>` entries
...
13a. Materialize remote rule-providers and expand common local/inline RULE-SET entries before sending config to the Extension. Done.
14. Add optional CMake-triggered Rust static library build for DevEco GUI builds. Done.
15. Produce and verify an actual OHOS Rust static library artifact in a DevEco/HarmonyOS SDK environment. Done.
...
17. Extend the Clash adapter to Hysteria2, TUIC, unresolved dynamic RULE-SET forms, and full Clash rule parity. Hysteria2/TUIC initial TCP/UDP paths, dat-backed GEOSITE Domain/Full/Plain entries, and common local/inline RULE-SET expansion are done; regex-only GEOSITE entries remain limited by the embedded matcher.
18. Replace the basic TCP latency probe with full Clash URL-test behavior, and wire real traffic/connections from the Extension core. URL-test probing is done for HTTP/HTTPS GET status validation through the selected proxy chain; connection list data remains placeholder.
...
- Latency tests work through Extension native-core while VPN is connected and through local native-core proxy-chain probes while disconnected. Disconnected tests must not start a UI-process mihomo runtime.
