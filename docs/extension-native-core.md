...
- The adapter supports `direct`, Shadowsocks, Snell, AnyTLS, NaiveProxy, SOCKS5, HTTP, VMess, VLESS, Trojan, and TLS/WS/Reality/ShadowTLS/v2ray-plugin WebSocket wrapping for selected nodes. It handles common Clash block options such as `ws-opts.path`, `ws-opts.headers.Host`, `reality-opts.public-key`, `reality-opts.short-id`, `reality-opts.server-name`, ShadowTLS `plugin-opts`, and v2ray-plugin `plugin-opts`.
- Flow-style proxy groups such as `proxies: [A, B, DIRECT]` are parsed without truncating at list commas. The ArkTS config parser uses the same nested-aware flow parsing for proxy display and provider expansion.
- Unsupported protocols or transports return explicit adapter errors. For example, `network: grpc`, `network: h2`, Hysteria2, and TUIC are not silently treated as plain TCP.
...
- The adapter maps common Clash rules to shoes TUN rules: `MATCH`, `DOMAIN`, `DOMAIN-SUFFIX`, `IP-CIDR`, `IP-CIDR6`, and `DST-PORT`.
- Unsupported routing rules that would materially change routing behavior, including `DOMAIN-KEYWORD`, `GEOIP`, `GEOSITE`, and unexpanded `RULE-SET`, fail explicitly instead of being silently ignored.
- Unsupported client protocols such as Hysteria2 and TUIC fail explicitly because the pinned shoes client config does not expose matching outbound variants.
