...
- Unsupported routing rules that the embedded shoes rule matcher still cannot model, including unexpanded/advanced `RULE-SET` forms and GEOSITE regex entries, are not represented as fake rules. The adapter currently skips them and relies on the Clash `MATCH` rule or generated default rule; status JSON reports `skippedRuleCount` and `skippedRuleTypes` so the UI/logs can explain partial rule coverage. Full parity requires backend rule-matcher work.
...
17. Extend the Clash adapter to Hysteria2, TUIC, unexpanded/advanced RULE-SET forms, and full Clash rule parity. Dat-backed GEOSITE Domain/Full/Plain entries are done; regex-only GEOSITE entries remain limited by the embedded matcher.
